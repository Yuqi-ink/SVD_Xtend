...
def interpolate_spherical(p0, p1, fract_mixing: float):
    r"""
    borrowed from latentblending repo
...
class StableVideoDiffusionPipeline_Custom(DiffusionPipeline):
    r"""
    ...
    def __call__(
        ...
        end_frame: Optional[Union[PIL.Image.Image, List[PIL.Image.Image], torch.FloatTensor]] = None,
        height: int = 576,
        width: int = 1024,
        ...
        if end_frame is not None:
            image_embeddings_end_frame = self._encode_image(end_frame, device, num_videos_per_prompt, do_classifier_free_guidance)
            emb_cond['start_frame'] = image_embeddings
            emb_cond['end_frame'] = image_embeddings_end_frame
        ...
        if end_frame is not None:
            end_frame = self.image_processor.preprocess(end_frame, height=height, width=width)
